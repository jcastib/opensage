...
public class FunctionDialog3 extends JDialog implements ActionListener, ItemListener, DocumentListener, KeyListener{//
 ...
 JLabel jLabel21 = new JLabel();
  ...
  boolean word_selected = false;
  boolean move_flag = false;

    public FunctionDialog3(Frame frame, String title, boolean modal) {
  ...
  private void jbInit() throws Exception {
    ...
    jBottomPanel.setBorder(new EmptyBorder(0,10,0,0));

      jCenterPanel.setLayout(borderLayout4);
      jCenterPanel.setPreferredSize(new Dimension(555,420));

      jCentertopPanel.setPreferredSize(new Dimension(555,220));
    jCentertopPanel.setLayout(borderLayout5);
      jCenterbottomPanel.setBorder(border5);
    jCenterbottomPanel.setPreferredSize(new Dimension(555,200));
    jCenterbottomPanel.setLayout(borderLayout10);
      jCentert1Panel.setBorder(border2);
    jCentert1Panel.setPreferredSize(new Dimension(555,112));
    jCentert1Panel.setLayout(borderLayout7);
      jCentert2Panel.setBorder(border3);
    jCentert2Panel.setPreferredSize(new Dimension(555,75));
    jCentert2Panel.setLayout(borderLayout6);
      jCentert3Panel.setPreferredSize(new Dimension(555,30));
    jCentert3Panel.setLayout(borderLayout9);

    buttonPanel.setLayout(borderLayout8);
    buttonPanel.setBorder(new EmptyBorder(0,1,0,2));
    ...
    jContinueRButton.setText("Quantitative");
...
jCentert2Panel.add(buttonPanel,  BorderLayout.EAST);
  ...
  protected void displayInfo(KeyEvent e)
  {
      ...
      if (id == KeyEvent.KEY_PRESSED)
      {
        ...
        int dot = Expression.getCaret().getDot();//Fetches the current position of the caret.
        int mark = Expression.getCaret().getMark();//Fetches the current position of the mark
        ...
        try {
          if (dot == mark) { //no selection
            if (keyCode == 37) { //Left
              if(move_flag)
              {
                Expression.setCaretPosition(dot-5);
                move_flag = false;
              }

              String next = Expression.getText(dot - 2, 2);

               if (next.compareTo(">>") == 0) {
                 Expression.requestFocus(true);
                 setWordSelection(dot-3);
                 move_flag = true;
               }
            }
            else if (keyCode == 39) { //Right
              String next = Expression.getText(dot, 2);

              if (next.compareTo("<<") == 0) {
                Expression.requestFocus(true);
                setWordSelection(dot+2);
              }
            }
          }
          else { //selection
            if (keyCode == 37) { //Left
              ...
              String next = Expression.getText(dot - 2, 2);
              if (next.compareTo("<<") == 0) {
                Expression.setCaretPosition(dot-2);
              }
            } else if (keyCode == 39) { //Right
              String next = Expression.getText(dot, 2);
  ...
  protected void setWordSelection(int offset)
  {
    word_selected = false;
    try{
      ...
      final int right_p = textb.indexOf(">>",start);
      final int left_p = textb_rev.indexOf("<<",textlength-end);

      if (right_p != -1 && left_p != -1)
      {
        final String total_word = textb.substring(textlength-left_p- 2, right_p+2);
  ...
  private void init_tree()
  {
    ...
    OidNode mean_var = new OidNode(listModel6,"Mean Variance Adjustment");
    IconNode Mean  = new IconNode(mean_var,"Folder");
    Functions.add(Mean);

    DefaultListModel listModel7 = new DefaultListModel();
    listModel7.addElement("<All>");
    OidNode out = new OidNode(listModel7,"Outlier/Winsorizing");
  ...
  Hashtable makeIcons(){
    ImageIcon folder = new ImageIcon(FunctionDialog3.class.getResource("Folder_small.png"));
    ImageIcon folder_plus = new ImageIcon(FunctionDialog3.class.getResource("folder_plus.png"));
    ImageIcon folder_minus = new ImageIcon(FunctionDialog3.class.getResource("folder_minus.png"));
    ImageIcon folder_open = new ImageIcon(FunctionDialog3.class.getResource("Folder_open.png"));
