...
public class MyPDFSearchFrame
  ...
  protected PdfDecoder2 decode_pdf;
  protected PdfDecoder decode_pdf_original;
  ...
  /**actual viewer object*/
  static MyPDFSearchFrame current;

  //<start-os>
  /**search frame*/
  JFrame searchFrame;

  /**flag to stop multiple listeners on search button*/
  ...
  protected float[] scalingFloatValues = {
      .25f, .5f, .75f, 1.0f, 1.25f, 1.5f, 2.0f, 2.5f, 5.0f, 7.5f, 10.0f};
  ...
  private final String[] annotTypes = {
      "Other", "Text", "FileAttachment"}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  private int minimumScreenWidth = 775;
  ...
  protected void openDefaultFile(String defaultFile) {
    ...
    if (defaultFile != null) {
      ...
      if (!testExists.exists()) {
        ...
        //                              defaultFile + "\n" + "does not exist.");//getMessage("PdfViewerdoesNotExist.message"));
      }
      else if (testExists.isDirectory()) {
        //##Token:PdfViewerFileIsDirectory.message=This file is a Directory and cannot be opened
        //JOptionPane.showMessageDialog(this,
        //                              defaultFile + "\n" + "This file is a Directory and cannot be opened.");//getMessage("PdfViewerFileIsDirectory.message"));
      }
      else {

        selectedFile = defaultFile;
        setViewerTitle("S.A.G.E. User Manual");
          ...
          try {
            ...
            if (pageNum < 1) {
              ...
              System.err.println(page +
                                 " must be 1 or larger. Opening on page 1");
            ...
            System.err.println(page +
                "is not a valid number for a page number. Opening on page 1");
            LogWriter.writeLog(page +
                "is not a valid number for a page number. Opening on page 1");
          }
        }
   ...
   */
  public MyPDFSearchFrame(Frame1 parent, String filename) {
    Toolkit t = Toolkit.getDefaultToolkit();
    this.setIconImage(t.createImage(MyPDFSearchFrame.class.getResource("SAGE.PNG")));
    this.parent = parent;
    current = this;
    current.setupViewer(filename);
    addKeyListener();
    //setSize(600, 800);
  }
  ...
  void addKeyListener()
  {
      getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).//WHEN_IN_FOCUSED_WINDOW).
              put(KeyStroke.getKeyStroke(KeyEvent.VK_PAGE_DOWN, 0), "pagedownPressed");

      getRootPane().getActionMap().
              put("pagedownPressed", new AbstractAction("pagedownPressed") {
      ...
      getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).//WHEN_IN_FOCUSED_WINDOW).
              put(KeyStroke.getKeyStroke(KeyEvent.VK_PAGE_UP, 0), "pageupPressed");

      getRootPane().getActionMap().
              put("pageupPressed", new AbstractAction("pageupPressed") {
          public void actionPerformed(ActionEvent actionEvent) {
             back(1);
          }
      });
  ...
  protected void init(ResourceBundle bundle) {
     ...
     */
    decode_pdf = new PdfDecoder2(); //USE THIS FOR THE VIEWER ONLY
    decode_pdf_original = new PdfDecoder();
     ...
     */
    decode_pdf.createPageHostspots(annotTypes,
                                   "org/jpedal/examples/simpleviewer/annots/"); //$NON-NLS-1$

    decode_pdf_original.createPageHostspots(annotTypes,
                                   "org/jpedal/examples/simpleviewer/annots/"); //$NON-NLS-1$
    /**
     * ANNOTATIONS code 2
     *
    ...
    try {
      ...
      decode_pdf_original.setDefaultDisplayFont("SansSerif");
    }
     ...
     // System.out.println("Fonts available are:-");//getMessage("PdfViewerFontsFound.message"));
      //System.out.println("=====================\n"); //$NON-NLS-1$
      ...
      for (int i = 0; i < count; i++) {
        ...
        //                   "Postscript = "+//getMessage("PdfViewerFontsPostscript.message") + "=" +
        //                   f.getPSName() + ")"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    ...
    //<start-os>
    /**
     * FONT EXAMPLE - Use fonts placed in jar for substitution (1.4 only)
     *
     ...
     * setTTFontDirs(String[] fontDirs) will achieve the same.
     *
     *
    ...
    /**create scrollpanes*/
    bookmarkScrollPane.setHorizontalScrollBarPolicy(JScrollPane.
        HORIZONTAL_SCROLLBAR_AS_NEEDED);
    bookmarkScrollPane.setVerticalScrollBarPolicy(JScrollPane.
                                                  VERTICAL_SCROLLBAR_AS_NEEDED);
    ...
    displayPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, navOptionsPanel,
                                 scrollPane);
   ...
   */
/*  private void extractSelectedScreenAsImage() {
    ...
    final BufferedImage snapShot = decode_pdf.getSelectedRectangleOnscreen(t_x1,
        t_y1, t_x2, t_y2, 100);
    ...
    final JDialog displayFrame = new JDialog(this, true);
    ...
    displayFrame.setLocation(this.getLocationOnScreen().x + 10,
                             this.getLocationOnScreen().y + 10);
    displayFrame.setSize(imgSize, imgSize);
    displayFrame.setTitle("Extracted Image - would you like to save?");
    ...
    yes.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        //options to save as
        String[] png = new String[] {
            "png"};
        String[] tiff = new String[] {
            "tif", "tiff"};
        String[] jpg = new String[] {
            "jpg", "jpeg"};

        //add general filters
        fc.addChoosableFileFilter(new FileFilterer(tiff, "TIFF"));
        fc.addChoosableFileFilter(new FileFilterer(jpg, "JPEG"));

        //In response to a button click:
        int returnVal = fc.showSaveDialog(image_scroll);
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
          ...
          String target = file.getPath() + "." + format;

          //Do the actual save
          if (snapShot != null)
            JAI.create("filestore", snapShot, target, format);
...
/*  private void createUniqueAnnontationIcons() {
  ...
  private void readChildNodes(Node rootNode, DefaultMutableTreeNode topNode) {
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if ( (rawDest != null) && (rawDest.indexOf("/XYZ") != -1)) { //$NON-NLS-1$
        ...
        pointLookupTable.put(title,
                             new Point( (int) Float.parseFloat(x),
  ...
  private void createOutlinePanels() {
    ...
    /*if (showThumbnails) {
      ...
      navOptionsPanel.add(thumbnails.setupThumbnails(pages, textFont,
          getMessage("PdfViewerPageLabel.text"), decode_pdf.getPdfPageData())
    ...
    if (decode_pdf.hasOutline() && showOutlines) {
      ...
      Node rootNode = decode_pdf_original.getOutlineAsXML().getFirstChild();
      ...
      if (rootNode != null) {
        ...
        navOptionsPanel.add(bookmarkScrollPane,
                            "Bookmarks");//getMessage("PdfViewerTitle.bookmarks"));
          ...
          public void valueChanged(TreeSelectionEvent e) {
            ...
            if ( (page != null) && (page.length() > 0)) {
              ...
              if ( (!isProcessing) && (currentPage != pageToDisplay)) {
                ...
                decode_pdf_original.setPageParameters(scaling, currentPage);
                decodePage(false);
              }
              ...
              if (p != null)
              {
                ...
                decode_pdf_original.ensurePointIsVisible(p);
              }
            }
            else
  ...
  protected int[] updateXY(MouseEvent event) {
    ...
    if (maxViewY != 0) { // will not be zero if viewport in play
      ...
      y = (int) ( (mediaH - ( (mediaH - (y / scaling) - dy) / viewportScale)) *
                 scaling);
    }
  ...
  protected class mouse_mover
    ...
    protected void scrollAndUpdateCoords(MouseEvent event) {
      ...
      Rectangle visible_test = new Rectangle(event.getX(), event.getY(),
                                             interval, interval);
      if ( (allowScrolling) && (!decode_pdf.getVisibleRect().contains(visible_test)))
      {
        decode_pdf.scrollRectToVisible(visible_test);
        decode_pdf_original.scrollRectToVisible(visible_test);
    ...
    protected void generateNewCursorBox() {
      ...
      if ( (old_m_x2 != -1) | (old_m_y2 != -1) | (Math.abs(m_x2 - old_m_x2) > 5) |
          (Math.abs(m_y2 - old_m_y2) > 5)) {
        ...
        decode_pdf_original.updateCursorBoxOnScreen(currentRectangle, Color.blue);
        ...
        decode_pdf_original.setHighlightedAreas(highlightedAreas);
        //reset tracking
        old_m_x2 = m_x2;
        old_m_y2 = m_y2;
  ...
  protected class mouse_clicker
    ...
    public void mouseReleased(MouseEvent event) {
     ...
     // }
      ...
      decode_pdf_original.updateCursorBoxOnScreen(null, null); //remove box
      decode_pdf.setHighlightedAreas(null); //remove highlighted text
      decode_pdf_original.setHighlightedAreas(null);

      decode_pdf.repaintArea(new Rectangle(m_x1 - cropX, m_y2 + cropY,
                                           m_x2 - m_x1 + cropX,
                                           (m_y1 - m_y2) + cropY), mediaH); //redraw
      decode_pdf_original.repaintArea(new Rectangle(m_x1 - cropX, m_y2 + cropY,
                                           m_x2 - m_x1 + cropX,
                                           (m_y1 - m_y2) + cropY), mediaH); //redraw

      decode_pdf.repaint();
      decode_pdf_original.repaint();
    }
  }
...
//    Rectangle[] hotSpots = decode_pdf.getPageHotspots();
    Rectangle[] hotSpots = decode_pdf_original.getPageHotspots();
        ...
        if (mouseClicked) {
          ...
          if ( (subtype.equals("Link")) && (annotAction != null)) { //$NON-NLS-1$
            ...
            JOptionPane.showMessageDialog(this, details,
                                          "Annotation Properties",//getMessage("PdfViewerTitle.annots"),
            ...
            if (title == null)
              ...
              title = "No Title";//getMessage("PdfViewerAnnots.notitle");

            String contents = pageAnnotations.getField(matchFound, "Contents"); //$NON-NLS-1$
            if (contents == null)
              //##Token:PdfViewerAnnots.nocont=No Contents
              contents = "No Contents";//getMessage("PdfViewerAnnots.nocont");
            JOptionPane.showMessageDialog(this, new TextArea(contents), title,
                                          JOptionPane.PLAIN_MESSAGE);
            ...
            if (fileDetails != null) {
              ...
              if (file == null)
                ...
                JOptionPane.showMessageDialog(this,
                                              "No File Embedded");//getMessage("PdfViewerAnnots.nofile"));
              else {
                /**
            ...
            JOptionPane.showMessageDialog(
                ...
                this, details, "Unimplemented subtype"//getMessage("PdfViewerAnnots.nosubtype") //$NON-NLS-1$
                + " " + subtype, JOptionPane.PLAIN_MESSAGE); //$NON-NLS-1$
          }
        }
        else
          //##Token:PdfViewerAnnots.entered=Entered link
          message = "Entered link"+ " " + matchFound;//getMessage("PdfViewerAnnots.entered") + " " + matchFound; //$NON-NLS-2$
      }
    }
  }
   ...
   */
  protected final JScrollPane createPane(JTextPane text_pane, String content,
                                         boolean useXML) throws
    ...
    text_pane.setToolTipText("Text can be selected and copied to clipboard");//getMessage("PdfViewerTooltip.text"));
    Document doc = text_pane.getDocument();
    //##Token:PdfViewerTitle.text=Extracted Content
    text_pane.setBorder(BorderFactory.createTitledBorder(new EtchedBorder(),
        "Extracted Content"));//getMessage("PdfViewerTitle.text")));
    ...
    if ( (useXML) && (content != null)) {
      ...
      while (data_As_tokens.hasMoreTokens()) {
        ...
        if ( (next_item.equals("<")) && ( (data_As_tokens.hasMoreTokens()))) { //$NON-NLS-1$
          ...
          String current_token = next_item + data_As_tokens.nextToken() +
    ...
    text_scroll.setHorizontalScrollBarPolicy(JScrollPane.
                                             HORIZONTAL_SCROLLBAR_NEVER);
    text_scroll.setVerticalScrollBarPolicy(JScrollPane.
                                           VERTICAL_SCROLLBAR_AS_NEEDED);
    return text_scroll;
  }
  ...
  public void updateCords(MouseEvent event) {
    ...
    if (maxViewY != 0) { // will not be zero if viewport in play
      ...
      ey = (int) ( (mediaH - ( (mediaH - (ey / scaling) - dy) / viewportScale)) *
                  scaling);
    }
    ...
    if ( (allowScrolling) && (!event.isShiftDown())) {
      ...
      Rectangle visible_test = new Rectangle(event.getX() - interval,
                                             event.getY() - interval,
...
//		 		##Token:PdfViewerCloseing.message=Are you sure you want to close?
        ...
        if (confirm == 0) {
          ...
          //System.exit(0);
          setVisible(false);
          Frame1.mainFrame1.isPDFViewExist = false;
/*        }
      }
      else {
//		 		##Token:PdfViewerDecodeWait.message=Please wait for page to display.
  ...
  protected void setupGUI() {
     ...
     */
    //<start-os>
    //##Token:PdfViewerCom.message=JPedal GUI Demo - version
    setViewerTitle("S.A.G.E. User Manual");
    /**
                     //<end-os>
    ...
    decode_pdf_original.addMouseMotionListener(new mouse_mover());
    decode_pdf_original.addMouseListener(new mouse_clicker());
    ...
    currentBar1.setBorder(BorderFactory.createEmptyBorder(5,0,5,0));//top, left, bottom, right
    currentBar1.setLayout(new FlowLayout(FlowLayout.LEADING, 2, 0));
     ...
     */
    createPageSizingButtons(currentBar1);
  ...
  protected void setupScreen() {
    ...
    int width = d.width / 2, height = d.height;
    ...
    //<start-13>
    //centre on screen
    setLocationRelativeTo(Frame1.mainFrame1);
    /*<end-13>
  ...
  protected void createPageSizingButtons(JToolBar currentBar2) {
    ...
    JLabel plus = new JLabel("Scaling");//getMessage("PdfViewerTooltip.scaling"));
    currentBar2.add(plus);
    currentBar2.add(Box.createHorizontalGlue());
    //##Token:PdfViewerTooltip.zoomin=zoom in
    plus.setToolTipText("zoom in");//getMessage("PdfViewerTooltip.zoomin"));
...
/*    openButton(currentBar1);
    ...
    ImageIcon printIcon = new ImageIcon(ClassLoader.getSystemResource(
        "org/jpedal/examples/simpleviewer/print.gif")); //$NON-NLS-1$
    ...
    printButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        if (printingThreads > 0)
          ...
          JOptionPane.showMessageDialog(c,
                                        getMessage("PdfViewerPrintWait.message"));
          ...
          if (printingThreads == 0)
            ...
            JOptionPane.showMessageDialog(c,
                                          getMessage("PdfViewerPrintFinish.message"));
        }
        else
    ...
    ImageIcon bookmarksIcon = new ImageIcon(ClassLoader.getSystemResource(
        "org/jpedal/examples/simpleviewer/bookmarks.gif")); //$NON-NLS-1$
    ...
    ImageIcon searchIcon = new ImageIcon(MyPDFSearchFrame.class.getResource("search.png")); //$NON-NLS-1$
    menuSearchButton.setIcon(searchIcon);
    createPressedLook(menuSearchButton, searchIcon);
    //##Token:PdfSearch.tip=Search the current PDF file
    menuSearchButton.setToolTipText("Search the current PDF file");//getMessage("PdfSearch.tip")); //$NON-NLS-1$
    currentBar1.add(menuSearchButton);
...
//        createSearchFrame();
        ...
        if (selectedFile == null) {
          JOptionPane.showMessageDialog(c, "No file open");
        }
        else if ( (searchFrame == null) || (!searchFrame.isVisible())) {

          //pop up new window to search text

          final JPanel nav = new JPanel();
          nav.setLayout(new BorderLayout());

          searchFrame = new JFrame();
          searchFrame.setIconImage(Toolkit.getDefaultToolkit().createImage(MyPDFSearchFrame.class.getResource("search_title.png")));
          ...
          searchAll.setText("Search whole document");
          nav.add(searchAll, BorderLayout.NORTH);

          itemFoundCount = 0;
          searchCount = new JTextField("Items found " + itemFoundCount);
          ...
          results.addListSelectionListener(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent event) {
              if (!event.getValueIsAdjusting()) {
                ...
                decode_pdf_original.setFoundTextAreas(null);
                ...
                if (id != -1) {
                  ...
                  if (newPage != null) {
                    ...
                    decode_pdf_original.invalidate();
                    decode_pdf_original.repaint();

                    //draw rectangle
                    int scrollInterval = decode_pdf.getScrollInterval();
                    //previous one to revert back to but other more accurate
//		    decode_pdf.scrollRectToVisible(new Rectangle((int)((highlight.x*scaling)+scrollInterval),(int)(cropH-((highlight.y-cropY)*scaling)-scrollInterval*2),scrollInterval*4,scrollInterval*6));

                    int x = (int) ( (highlight.x - cropX) * scaling) + inset;
                    int y = (int) ( (cropH - (highlight.y - cropY)) * scaling) +
                        inset;
                    int w = (int) (highlight.width * scaling);
                    int h = (int) (highlight.height * scaling);

                    Rectangle scrollto = new Rectangle(x - scrollInterval,
                        y - h - scrollInterval, w + scrollInterval * 2,
                        h + scrollInterval * 2);
                    decode_pdf.scrollRectToVisible(scrollto);
                    decode_pdf.setFoundTextAreas(highlight);

                    decode_pdf_original.scrollRectToVisible(scrollto);
                    decode_pdf_original.setFoundTextAreas(highlight);
                  }
                }
              }
            }
          ...
          textPages = new HashMap();
          textRectangles = new HashMap();

          //setup searching
          if (firstTime) { //global variable so do NOT reinitialise
            firstTime = false;
          ...
          searchFrame.getContentPane().add(nav, BorderLayout.NORTH);

          //position and size
          int w = 200;
          int h = current.getHeight();
          int x1 = current.getLocationOnScreen().x;
          int x = current.getWidth() + x1;
          int y = current.getLocationOnScreen().y;
          Dimension d = Toolkit.getDefaultToolkit().getScreenSize();
          int width = d.width;
          if (x + w > width) {
            x = width - w;
            current.setSize(x - x1, current.getHeight());
          }

          searchFrame.setSize(w, h);
          searchFrame.setLocation(x, y);
          //<start-13>
          searchAll.setFocusable(false);
          //<end-13>
          searchText.grabFocus();
          searchFrame.setVisible(true);//.show();
        }
        else
          searchText.grabFocus();
  ...
  protected void createSearchFrame()
  {
    //if (selectedFile == null) {
    //  JOptionPane.showMessageDialog(c, "No file open");
    //}
...
//			    			    			   decode_pdf.scrollRectToVisible(new Rectangle((int)((highlight.x*scaling)+scrollInterval),(int)(cropH-((highlight.y-cropY)*scaling)-scrollInterval*2),scrollInterval*4,scrollInterval*6));
      ...
      searchButton.addActionListener(this);//(new ActionListener() {
...
//        searchText.grabFocus();
  ...
  {
    //if(e.getSource() == menuSearchButton)
    //{
    //  showSearchFrame();
    //}
  ...
  protected void clickSearchButton()
  {
    if (!isSearch) {
      ...
      searcher.interrupt();
      //System.out.println("interrupt search button!");
      isSearch = false;
      searchButton.setText("search");
    }
  }
...
/*  protected void openButton(JToolBar currentBar1) {
    ...
    ImageIcon openIcon = new ImageIcon(ClassLoader.getSystemResource(
        "org/jpedal/examples/simpleviewer/open.gif")); //$NON-NLS-1$
...
/*  protected void createPropertyIcon(JToolBar currentBar1) {
    ...
    ImageIcon propIcon = new ImageIcon(ClassLoader.getSystemResource(
        "org/jpedal/examples/simpleviewer/properties.gif")); //$NON-NLS-1$
...
/*  protected void createAboutButton(JToolBar currentBar1) {
    ...
    ImageIcon aboutIcon = new ImageIcon(ClassLoader.getSystemResource(
        "org/jpedal/examples/simpleviewer/about.gif")); //$NON-NLS-1$
...
/*  protected void createFontButton(JToolBar currentBar1) {
    ...
    ImageIcon fontIcon = new ImageIcon(ClassLoader.getSystemResource(
        "org/jpedal/examples/simpleviewer/font.gif")); //$NON-NLS-1$
  ...
  protected void createPageNavigation(JToolBar currentBar1) {
    ...
    first.setIcon(new ImageIcon(MyPDFSearchFrame.class.getResource("back1.png")));
    createPressedLook(first, new ImageIcon(MyPDFSearchFrame.class.getResource("back1.png")));
    //##Token:PdfViewerRewindToStart=Rewind to page 1
    first.setToolTipText("Rewind to page 1");//getMessage("PdfViewerRewindToStart"));
    ...
    fback.setIcon(new ImageIcon(MyPDFSearchFrame.class.getResource("back2.png")));
    createPressedLook(fback, new ImageIcon(MyPDFSearchFrame.class.getResource("back2.png")));
    //##Token:PdfViewerRewind10=Rewind 10 pages
    fback.setToolTipText("Rewind 10 pages");//getMessage("PdfViewerRewind10"));
    ...
    back.setIcon(new ImageIcon(MyPDFSearchFrame.class.getResource("back3.png")));
    createPressedLook(back, new ImageIcon(MyPDFSearchFrame.class.getResource("back3.png")));
    //##Token:PdfViewerRewind1=Rewind one page
    back.setToolTipText("Rewind one page");//getMessage("PdfViewerRewind1"));
    ...
    forward.setIcon(new ImageIcon(MyPDFSearchFrame.class.getResource("forward3.png")));
    createPressedLook(forward, new ImageIcon(MyPDFSearchFrame.class.getResource("forward3.png")));
    //##Token:PdfViewerForward1=forward 1 page
    forward.setToolTipText("Forward 1 page");//getMessage("PdfViewerForward1"));
    ...
    fforward.setIcon(new ImageIcon(MyPDFSearchFrame.class.getResource("forward2.png")));
    createPressedLook(fforward, new ImageIcon(MyPDFSearchFrame.class.getResource("forward2.png")));
    //##Token:PdfViewerForward10=Fast forward 10 pages
    fforward.setToolTipText("Fast forward 10 pages");//getMessage("PdfViewerForward10"));
    ...
    end.setIcon(new ImageIcon(MyPDFSearchFrame.class.getResource("forward1.png")));
    createPressedLook(end, new ImageIcon(MyPDFSearchFrame.class.getResource("forward1.png")));
    //##Token:PdfViewerForwardLast=Fast forward to last page
    end.setToolTipText("Fast forward to last page");//getMessage("PdfViewerForwardLast"));
    currentBar1.add(end);
    end.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        if ( (selectedFile != null) && (pageCount > 1) &&
  ...
  protected void initGUIComponents() {
    ...
    final String[] scalingValues = {
        "Window", //getMessage("PdfViewerScaleWindow.text"),
        "Height",//getMessage("PdfViewerScaleHeight.text"),
        //##Token:PdfViewerScaleWidth.text=Width
        "Width",//getMessage("PdfViewerScaleWidth.text"),
        "25", "50", "75", "100", "125", "150", "200", "250", "500", "750",
        "1000"}; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$ //$NON-NLS-8$ //$NON-NLS-9$ //$NON-NLS-10$ //$NON-NLS-11$
    this.scalingValues = scalingValues;
    float[] scalingFloatValues = {
        1.0f, 1.0f, 1.0f, .25f, .5f, .75f, 1.0f, 1.25f, 1.5f, 2.0f, 2.5f, 5.0f,
        7.5f, 10.0f};
    ...
    pageCounter1 = new JLabel("Page");//getMessage("PdfViewerPageLabel.text"));
    ...
    pageCounter2.setToolTipText("To go to a page - Type in page number and press return");//getMessage("PdfViewerTooltip.goto"));
    pageCounter2.setBorder(BorderFactory.createLineBorder(Color.black));
    pageCounter2.addActionListener(this);//new ActionListener() {
     ...
     //   setPage(value);
        ...
        try {
          ...
          if ( (newPage > decode_pdf.getPageCount()) | (newPage < 1)) {
            //##Token:PdfViewerPageLabel.text=Page
            JOptionPane.showMessageDialog(c,
                                          getMessage("PdfViewerPageLabel.text") +
                                          " " + //$NON-NLS-2$
                                          //##Token:PdfViewerOutOfRange.text=is not in range. Total pagecount=
                                          value + " " +
                                          getMessage("PdfViewerOutOfRange.text") +
                                          " " + decode_pdf.getPageCount()); //$NON-NLS-1$  //$NON-NLS-3$
            newPage = currentPage;
          ...
          JOptionPane.showMessageDialog(c,
                                        ">" + value + "< " + getMessage("PdfViewerInvalidNumber.text")); //$NON-NLS-1$ //$NON-NLS-2$
          newPage = currentPage;
          pageCounter2.setText("" + currentPage); //$NON-NLS-1$
        }
    ...
    pageCounter3 = new JLabel("of");//getMessage("PdfViewerOfLabel.text") + " ");
    pageCounter3.setOpaque(false);

    scalingBox.setBackground(Color.white);
    scalingBox.setEditable(true);
    scalingBox.setPreferredSize(new Dimension(65, 20));
  ...
  public void setPage(String value)
  {
    ...
    try {
      ...
      if ( (newPage > decode_pdf.getPageCount()) | (newPage < 1)) {
        //##Token:PdfViewerPageLabel.text=Page
        JOptionPane.showMessageDialog(c,
                                      "Page"+//getMessage("PdfViewerPageLabel.text") +
                                      " " + //$NON-NLS-2$
                                      //##Token:PdfViewerOutOfRange.text=is not in range. Total pagecount=
                                      value + " " +
                                      "is not in range. Total pagecount="+//getMessage("PdfViewerOutOfRange.text") +
                                      " " + decode_pdf.getPageCount()); //$NON-NLS-1$  //$NON-NLS-3$
        newPage = currentPage;
        pageCounter2.setText("" + currentPage); //$NON-NLS-1$
      }
      ...
      //##Token:PdfViewerInvalidNumber.text=is not valid. Please use a number (ie 4)
      JOptionPane.showMessageDialog(c,
                                    ">" + value + "< " + "is not valid. Please use a number (ie 4)");//getMessage("PdfViewerInvalidNumber.text")); //$NON-NLS-1$ //$NON-NLS-2$
      newPage = currentPage;
      pageCounter2.setText("" + currentPage); //$NON-NLS-1$
    }
  ...
  protected void createMenu(JMenuBar currentMenu, boolean includeAll) {
    ...
    JMenu fileMenuList = new JMenu("File");//getMessage("PdfViewerFile.text"));
  ...
  private void optionAutoscroll(JMenu options) {
    ...
    JMenuItem autoScroll = new JMenuItem("Toggle autoscrolling");//getMessage("PdfViewerTitle.autoscroll"));
    //##Token:PdfViewerTooltip.autoscroll=Stops screen scrolling when reached edge of viewport
    autoScroll.setToolTipText("Stops screen scrolling when reached edge of viewport");//getMessage("PdfViewerTooltip.autoscroll"));
  ...
  private void exitOption(JMenu file) {
    ...
    JMenuItem exit = new JMenuItem("Close");//getMessage("PdfViewerExit.text"));
    //##Token:PdfViewerShutdown.text=Shut down the program
    exit.setToolTipText("Close the Help Frame");//getMessage("PdfViewerShutdown.text"));
    exit.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        if (printingThreads > 0)
          //##Token:PdfViewerStillPrinting.text=Pages still printing
          JOptionPane.showMessageDialog(c,
                                        "Pages still printing");//getMessage("PdfViewerStillPrinting.text"));
        else
          exit();
      }
    });
    ...
    psize.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        if (currentPageSize != null) {
          ...
          JLabel g1 = new JLabel(getMessage("PdfViewermediaBox.text") +
                                 currentPageSize.getMediaValue(currentPage));
          g1.setFont(textFont);
          details.add(g1);
          details.add(Box.createRigidArea(new Dimension(0, 5)));

          //##Token:PdfViewercropBox.text=Crop Box :
          JLabel g2 = new JLabel(getMessage("PdfViewercropBox.text") +
                                 currentPageSize.getCropValue(currentPage));
          g2.setFont(textFont);
          details.add(g2);
          details.add(Box.createRigidArea(new Dimension(0, 5)));

          //##Token:PdfViewerrotation.text=Rotation :\
          JLabel g3 = new JLabel(getMessage("PdfViewerrotation.text") +
                                 currentPageSize.getRotation(currentPage));
          g3.setFont(textFont);
          details.add(g3);
          ...
          JOptionPane.showMessageDialog(c, details,
                                        getMessage("PdfViewersize.text"),
                                        JOptionPane.PLAIN_MESSAGE);
        }
        else
          ...
          //##Token:PdfViewerTooltip.pageSize=Internal page size of current page
          JOptionPane.showMessageDialog(c,
                                        getMessage("PdfVieweremptyFile.message"),
                                        getMessage("PdfViewerTooltip.pageSize"),
                                        JOptionPane.PLAIN_MESSAGE);
...
/*  private void saveOption(JMenu file) {
    ...
    save.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        chooser.addChoosableFileFilter(new FileFilterer(new String[] {"pdf"},
            "Pdf (*.pdf)")); //$NON-NLS-1$
        chooser.setFileSelectionMode(JFileChooser.FILES_ONLY);
...
*/
  ...
  /*protected void showPropertiesBox() {
    ...
    PdfFileInformation currentFileInformation = decode_pdf.
    ...
    if (currentFileInformation != null) {
      ...
      JOptionPane.showMessageDialog(
          //##Token:PdfViewerMenu.props=Document Properties
          this, details, getMessage("PdfViewerMenu.props"),
      ...
      JOptionPane.showMessageDialog(
          //##Token:PdfVieweremptyFile.message=No file data available
          //##Token:PdfViewerMenu.props=Document Properties
          this, getMessage("PdfVieweremptyFile.message"),
          getMessage("PdfViewerMenu.props"), JOptionPane.PLAIN_MESSAGE);
  ...
  /*private void showFontsBox() {
    ...
    JOptionPane.showMessageDialog(
        ...
        this, details, getMessage("PdfViewerFontList.subtitle"),
        JOptionPane.PLAIN_MESSAGE);
  }*/
 ...
 /* protected void showInfoBox() {
    ...
    ImageIcon logo = new ImageIcon(ClassLoader.getSystemResource(
        "org/jpedal/examples/simpleviewer/logo.gif")); //$NON-NLS-1$
...
/*  protected void printDialog() {
      ...
      public void run() {
        ...
        try {
          ...
          while (selectPageSize) {
            ...
            String[] options = {
                getMessage("PdfViewerHelp.text"), getMessage("PdfViewera4"),
                ...
                getMessage("PdfVieweruseScaleOnPrinting.text"),
                getMessage("PdfViewerCancel.text"),
                //##Token:PdfViewerPrint.text=Print
            ...
            JTextArea message = new JTextArea(
                ...
                + "\n" + getMessage("PdfViewerPrintInfo5") +
                paper.getImageableX()
                + " " //$NON-NLS-1$
                ...
                + "\n\n" + getMessage("PdfViewerPrintScalingValue") +
                decode_pdf.getScaleForPrinting());
            ...
            int n = JOptionPane.showOptionDialog(c,
                                                 ...
                                                 message,
                                                 getMessage("PdfViewerPrintOptions.title"),
            ...
            switch (n) {
              case 0: //help
                ...
                JTextArea info = new JTextArea(getMessage(
                    "PdfViewerHelp.message"));
              ...
              case 2: //A4 (borderless)
                ...
                JOptionPane.showMessageDialog(c,
                                              getMessage("PdfViewerPrintWarning"));
                break;
        ...
        if (!canceled) //##Token:PdfViewerPrintingFinished=finished printing
          JOptionPane.showMessageDialog(c,
                                        getMessage("PdfViewerPrintingFinished"));
      }
    };
  ...
  public void selectFile() {
    ...
    if (file != null && state == JFileChooser.APPROVE_OPTION) {
      ...
      if (!testFile.exists()) {
        //##Token:PdfViewerFile.text=File
        JOptionPane.showMessageDialog(this,
                                      "File" + selectedFile//getMessage("PdfViewerFile.text") + selectedFile
                                      //##Token:PdfViewerNotExist=\ does not exist
                                      + "does not exist.");//getMessage("PdfViewerNotExist"));
        /**
         * open the file
         */
      }
        ...
        decode_pdf_original.flushObjectValues(true);

        //reset the viewableArea before opening a new file
        decode_pdf.resetViewableArea();
        decode_pdf_original.resetViewableArea();
      ...
      JOptionPane.showMessageDialog(this, "Nothing selected.");//getMessage("PdfViewerNoSelection"));
  ...
  public void setViewerTitle(final String title) {
    ...
    if (title != null) {
      ...
      if (titleMessage == null)
        setTitle("S.A.G.E. User Reference Manual"); //$NON-NLS-1$
      else
        setTitle(titleMessage + selectedFile); //$NON-NLS-1$
      /***********************************************************
               //<end-os>
              //##Token:PdfViewerOs.message=JPedal GUI Open Source version - version
  ...
  private void openFile(String selectedFile) {
    ...
    if (searchFrame != null) {
      searchFrame.setVisible(false);//.hide();
      searchFrame.dispose();
    }
    //<end-os>
  ...
  private boolean openUpFile(String selectedFile) {
    ...
    decode_pdf.closePdfFile();
    decode_pdf_original.closePdfFile();
    ...
    try {
     ...
     decode_pdf_original.openPdfFile(selectedFile);
      ...
      if ( (decode_pdf.isEncrypted()) && (!decode_pdf.isFileViewable())) {
        ...
        String password = JOptionPane
            ...
            .showInputDialog(this, "Please enter password");//getMessage("PdfViewerPassword.message")); //$NON-NLS-1$
        ...
        if (password != null) {
          ...
          decode_pdf_original.setEncryptionPassword(password);
          decode_pdf_original.verifyAccess();
      ...
      if (!fileCanBeOpened) {
        ...
        JOptionPane
            .showMessageDialog(this,
                               ...
                               "A valid password is required to display encrypted files.");//getMessage("PdfViewerPasswordRequired.message")); //$NON-NLS-1$
        //<end-13>
      ...
      JOptionPane
          .showMessageDialog(
              ...
              "This file generated an exception and cannot continue.");//getMessage("PdfViewerOpenerror")); //$NON-NLS-1$
      System.exit(1);
      //<end-example>
      isProcessing = false;
    }
  ...
  private void processPage() {
    ...
    if (switchModes) {
        ...
        decode_pdf_original.setRenderMode(PdfDecoder.RENDERIMAGES + PdfDecoder.RENDERTEXT);

        decode_pdf.useHiResScreenDisplay(false);
        decode_pdf_original.useHiResScreenDisplay(false);
        decode_pdf.setRenderMode(PdfDecoder.RENDERIMAGES);
                          decode_pdf_original.setRenderMode(PdfDecoder.RENDERIMAGES);

                          // if we want to use java 13 JPEG conversion
                          decode_pdf.setEnableLegacyJPEGConversion(true);
                  decode_pdf_original.setEnableLegacyJPEGConversion(true);
      /**
...
if(decode_pdf_original != null)
      {
          currentFileInformation = decode_pdf_original.getFileInformationData();
      }

      //switch all on by default

if(decode_pdf_original != null && currentFileInformation != null)
      {
          String[] values = currentFileInformation.getFieldValues();
          String[] fields = currentFileInformation.getFieldNames();
          ...
          for (int i = 0; i < fields.length; i++) {
            ...
            if (fields[i].equals("Creator")) {
              ...
              if (values[i].equals("XPP")) {
                ...
                decode_pdf_original.useHiResScreenDisplay(true);
    ...
    pageCount = decode_pdf_original.getPageCount();
    ...
    decode_pdf_original.setExtractionMode(0, 72, scaling);
    /***/
    ...
    decode_pdf_original.setPDFBorder(BorderFactory.createLineBorder(Color.black, 1));

    /** turn off border in printing */
    decode_pdf.disableBorderForPrinting();
    decode_pdf_original.disableBorderForPrinting();
    ...
    pageCounter2.setText(" " + currentPage); //$NON-NLS-1$
//##Token:PdfViewerOfLabel.text=of
    pageCounter3.setText("of" + " " + pageCount);//getMessage("PdfViewerOfLabel.text") + " " + pageCount);
    //<end-example>
  ...
  private void resetRotationBox() {
    ...
    PdfPageData currentPageData = decode_pdf_original.getPdfPageData();
    ...
    if (decode_pdf != null) {
      ...
      if (index != -1) {
        if (index < 3) { //handle scroll to width/height/window
          ...
          PdfPageData pageData = decode_pdf_original.getPdfPageData();
      ...
      if (decode_pdf != null) //allow for clicking on it before page opened
      {
        ...
        decode_pdf_original.setPageParameters(scaling, currentPage, rotation);
      }
      decode_pdf.invalidate();
      decode_pdf_original.invalidate();
      decode_pdf.repaint();
  ...
  public void forward(int count) {
    ...
    if (!isProcessing) { //lock to stop multiple accesses
      ...
      //##Token:PdfViewerDecodeWait.message=Please wait for page to display
      JOptionPane.showMessageDialog(this,
                                    "Please wait for page to display.");//getMessage("PdfViewerDecodeWait.message")); //$NON-NLS-1$
  }
  ...
  protected void decodePage(final boolean resizePanel) {
    ...
    decode_pdf_original.clearScreen();
      ...
      public Object construct() {
        ...
        try {
          ...
          try {
            ...
            decode_pdf_original.decodePage(currentPage);

            //read values for page display
            //PdfPageData page_data = decode_pdf.getPdfPageData();
            PdfPageData page_data = decode_pdf_original.getPdfPageData();
            ...
            pageAnnotations = decode_pdf_original.getPdfAnnotsData();
          ...
          pageCounter2.setText(" " + currentPage); //$NON-NLS-1$
          //##Token:PdfViewerOfLabel.text=of
          pageCounter3.setText("of"+" " + //getMessage("PdfViewerOfLabel.text") + " " +
                               pageCount); //$NON-NLS-1$
          //<end-example>
  ...
  public void setTextonPage(final String find_text, final boolean option, final int pagenumber)
  {
      /** if running terminate first */
      if ( (isProcessing) | (worker != null))
        worker.interrupt();
      ...
      worker = new SwingWorker() {
        public Object construct() {
          ...
          try {
            ...
            if ( (!isProcessing) && (currentPage != pagenumber)) {
        ...
        {
          try {
            searchText(find_text, option, pagenumber);
          }
          catch (Exception e1) {
            //System.out.println("Exception " + e1);
            e1.printStackTrace();
  ...
  public void exit() {
    ...
    /*JOptionPane.showConfirmDialog(
        ...
        this, new JLabel("Exiting program"),//getMessage("PdfViewerExiting")),
        //##Token:PdfViewerprogramExit=Program exit
        "Program exit",//getMessage("PdfViewerprogramExit"),
...
/*  protected String getMessage(String key) {
  ...
  private void initMessages() {
    ...
    try {
      ...
      while (true) {
        ...
        if (equalsIndex != -1)
          messages.put(line.substring(0, equalsIndex),
    ...
    searcher = new SwingWorker() {
      ...
      public Object construct() {
        ...
        try {
          ...
          PdfPageData pageSize = decode_pdf_original.getPdfPageData();
          ...
          for (int i = startPage; i < endPage; i++) {
            ...
            try {
              if (i == currentPage){
                currentGrouping = decode_pdf_original.getGroupingObject();
                //currentGrouping = getGroupingObject(decode_pdf);
              }
              else {
                decode_pdf_original.decodePageInBackground(i);
                currentGrouping = decode_pdf_original.getBackgroundGroupingObject();
                //currentGrouping = getBackgroundGroupingObject(decode_pdf);
              }
              ...
              float[] co_ords = currentGrouping.findTextInRectangle(x1, y2,
                  x2 + x1, y1, i, textToFind, false, true);
              ...
              if ( (co_ords != null) | ( (i % 16) == 0)) {
                searchCount.setText("Items found " + itemFoundCount + " Scanning:" + i);
          ...
          searchCount.setText("Items found " + itemFoundCount + " Done");
          results.invalidate();
          results.repaint();
  ...
  public void searchText(final String textToFind_input, final boolean range, final int pageindex) throws Exception {
    ...
    searcher = new SwingWorker() {
      ...
      public Object construct() {
        ...
        try {
          ...
          //get text
          String textToFind = textToFind_input.trim();
          ...
          if (id != -1) {
            ...
            if (newPage != null) {
             ...
             if (currentPage != nextPage) {
              ...
              int x = (int) ( (highlight.x - cropX) * scaling) + inset;
              int y = (int) ( (cropH - (highlight.y - cropY)) * scaling) + inset;
              int w = (int) (highlight.width * scaling);
              int h = (int) (highlight.height * scaling);
