...
public class Dialog3_Cov extends JDialog implements DocumentListener, ActionListener, ItemListener{//JInternalFrame
  ...
  public Dialog3_Cov(String title) {
  ...
  private void jbInit() throws Exception {
    ...
    {
        if (MakeParameterStep2.covariate_count == 1) {
            DataModel.setValue("covariate.missing", DefaultMissingValue);
        }
        MakeParameterStep2.covariate_count++;
        ...
        if (applynextCheckBox.isSelected() && applyTextField.getText().length() > 0)
        {
            ...
            final Vector CovList_Start_array = new Vector();
            final Vector CovList_End_array = new Vector();
            final Vector CovList_Start_Index_array = new Vector();
            final Vector CovList_End_Index_array = new Vector();
            ...
            SwingWorker worker = new SwingWorker() {
                ...
                public Object construct() {
                    ...
                    for (int i = 0; i <= count; i++) {
                        ...
                        if (count >= 5) {
                            if (i == 0) { //start
                                CovList_Start_array.add(HeaderNames);
                                CovList_Start_Index_array.add(Integer.toString(cindex+i));
                            }
                            if (i == count) { //end
                                CovList_End_array.add(HeaderNames);
                                CovList_End_Index_array.add(Integer.toString(cindex+i));
                            }
                          }
                    }

                    DataModel.setValue("CovList_Start_array", CovList_Start_array);
                    DataModel.setValue("CovList_End_array", CovList_End_array);
                    DataModel.setValue("CovList_Start_Index_array", CovList_Start_Index_array);
                    DataModel.setValue("CovList_End_Index_array", CovList_End_Index_array);
  ...
  public void SetDefaultData(DataCollectionModel DM, String HN, String ST, int columnindex) {
      ...
      if(DataModel.hasValue("covariate.missing"))
        defaultmissingvalue = DataModel.getValue("covariate.missing").toString();
